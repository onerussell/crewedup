...
{         
    ...
    require_once CLASS_PATH . 'Model/Main/Calendar.php';
    require_once CLASS_PATH . 'Model/Main/Model_Main_Calendar_Events.php';
    require_once CLASS_PATH . 'Model/Main/Model_Main_Comments.php';
    require_once CLASS_PATH . 'Model/Main/Model_Main_Resume.php';
    require_once CLASS_PATH . 'Model/Main/Model_Main_Award.php';    
    ...
    $gSmarty -> assign('lgear_cnt', $gGear -> GetCount($uid, 0));
    ...
    switch ($act)
    {
            ...
            switch($what)
            {
                ...
                case 'add':
                    ...
                    if (!empty($_POST['fm']))
                    {
                        ...
                        if (empty($fm['year']) || 'Year' == $fm['year'])
                            $fm['year'] = '';

                        if (empty($fm['year_f']) || 'Year' == $fm['year_f'])
                            $fm['year_f'] = '';

                        if (empty($fm['title']) || 'Project Title' == $fm['title'])
                            $errs[] = 'Please specify a project title';
                        
                        if (empty($fm['company']) || 'Production Company Name' == $fm['company'])
                            $errs[] = 'Please specify production company name';
                        
                        if ('resume' == $act && (empty($fm['prof']) || 'Job Title' == $fm['prof']))
                            $errs[] = 'Please specify a job title';
                        ...
                        if (empty($errs))
                        {
                            $sdate = (int)_va($fm,'year').'-'.(int)_va($fm,'month').'-01';
                            $fdate = (int)_va($fm,'year_f').'-'.(int)_va($fm,'month_f').'-01';
                            $ar = array(
                                       UID,
                                       trim(strip_tags(_va($fm,'title'))),
                                       trim(strip_tags(_va($fm,'prof'))),
                                       $sdate,
                                       trim(strip_tags(_va($fm, 'story'), $st)),
                                       trim(strip_tags(_va($fm,'company'))),
                                       
                                       trim(strip_tags(_va($fm,'prof_cat'))),
                                       $fdate,
                                       (int)_va($fm, 'prj_type', 4),
                                       trim(strip_tags(_va($fm, 'prj_episode'))),
                                       (int)_va($fm,'empl_type', 1)
                    ...
                    elseif ($id)
                    {
                        ...
                        if (!empty($fi))
                        {
                            $fm = array( 'title' => $fi['title'], 'story' => $fi['story'], 'month' => $fi['month'], 'year' => $fi['year'], 'company' => $fi['company'], 'prof' => $fi['prof'],'prof_cat' => $fi['prof_cat'], 'month_f' => $fi['month_f'], 'year_f' => $fi['year_f'],'prj_type' => $fi['prj_type'], 'prj_episode' => $fi['prj_episode'],'empl_type' => $fi['empl_type']); 
                            $gSmarty -> assign_by_ref('fm', $fm);
                        }             
                    }
                    else
                    ...
                    $gSmarty -> assign('prj_types', $gFm -> GetCatList());
                    

                    require_once '_prof_ar.php';
                ...
                case 'forward':
                    ...
                    if (!empty($_POST['fm']['email']))
                    {
                        ...
                        if (empty($errs))
                        {
                            ...
                            require CLASS_PATH.'Ctrl/Mail/libmail.php';
                            $m = new Mail;
        ...
        case 'comments':
            ...
            switch($what)
            {
                ...
                case 'addreply':
                    ...
                    if (IS_AUTH && $is_user && !empty($_POST['fm']['cid']) && !empty($_POST['fm']['story']))
                    {
        ...
        ****************************************/        
        ...
        // *****************************************
        case 'userdel':
            if ($is_user && !empty($uid)) {};
            $bc['Delete Profile'] = PATH_ROOT.'profile'.$uid.'/userdel';
            $del = (!empty($_REQUEST['del'])) ? $_REQUEST['del'] : '';
//            if ($del)
            switch ($del)
              {
              case '1':
                $userdel = $gUser->Delete($uid);
                if ($userdel)
                  uni_redirect(PATH_ROOT . 'index.php');
                else
                  uni_redirect(PATH_ROOT . 'profile.php'.$uid);
              break;
              
              default:
                uni_redirect(PATH_ROOT . 'userdel.php');
              break;
              }
            ...
            switch($ui['status'])
            {
                ...
                ****************************************/
                case 1:
                    /** get some comments */
                    $gCom =& new Model_Main_Comments($gDb, array('comments' => TB . 'comments', 'users' => TB . 'users'));
                    $gSmarty -> assign_by_ref('coml', $gCom -> GetList( $uid, 0, 0, 10) );
