<?php
/**
 * Простой класс для быстрой организации постраничного просмотра (VIEW)
 *
 * Пример вызова: 
 *
 * $this -> mPage -> GeneratePages('SELECT *  FROM test WHERE login = ?', -- запрос
 *                                  array('admin'), -- подстановочные переменные
 *                                  3,              -- номер записи, с которой начинать вывод тек страницы
 *                                  20)             -- количество записей на страницу
 *
 * вернет массив с двумя элементами: rows - записи, gp - цифры для отображения и номер текущей страницы
 *
 * Все в одном месте: сами записи и все, что нужно, чтобы сформировать список с номерами 
 * страниц. 
 *
 * @package    CrewedUp
 * @version    1.0
