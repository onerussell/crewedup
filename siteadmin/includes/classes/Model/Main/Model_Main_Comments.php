...
class Model_Main_Comments
{
    ...
    {
    	$this -> mDbPtr -> query('DELETE FROM '.$this -> mTbComments.' WHERE id = '.$id);
    	$this -> mDbPtr -> query('DELETE FROM '.TB.'wire WHERE ptype = 10 AND ext_id = '.$id);
    	return true;
    }/** Del */
    ...
    {
        $sql = 'SELECT COUNT(id) FROM '.$this -> mTbComments.' WHERE 1 ';
    	$ar = array();
        if (-1 != $ctype)
        {
            $sql .= ' AND ctype = ?';  
