...
class Model_Main_Blog
{
    ...
    {
    	// Delete items
    	$this -> mDbPtr -> query('DELETE FROM '.$this -> mTbBlogCom.' WHERE bid = '.$id);
    	
    	// Delete base
    	$this -> mDbPtr -> query('DELETE FROM '.$this -> mTbBlog.' WHERE id = '.$id);

    	$this -> mDbPtr -> query('DELETE FROM '.TB.'wire WHERE ptype = 1 AND ext_id = '.$id);
    	return true;
    }
    ...
    public function DelCom($id)
    {
    	...
    	if ($bid)
    	{
    	    ...
    	    $this -> mDbPtr -> query('DELETE FROM '.$this -> mTbBlogCom.' WHERE id = '.$id);
    	    $this -> mDbPtr -> query('DELETE FROM '.TB.'wire WHERE ptype = 11 AND id = '.$id);
    	}
