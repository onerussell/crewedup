<?php
/**
 * Ajax gate (only special for JsHttpRequest)
 *
 * @package   CrewedUp
 * @version   1.0
 * @since     14.05.2009
 ...
 */
    ...
    $JsHttpRequest =& new JsHttpRequest(DEF_CHARSET);
     
    // file_put_contents('tt.txt',serialize($_REQUEST). print_r($_REQUEST, true));
    	...
    	case 'get_data_list':
            ...
            switch($what)
            {
                ...
                case 'comments':
                    if (!empty($ui))
                    {
                        $gSmarty -> assign('euid', (int)_v('uid'));
