...
try
{
    switch ($mod)
    {
                ...
                case 2:
                    ...
                    if (isset($_POST['fm']) && 0 < count($_POST['fm']))
                    {
                        ...
                        try
                        {
                            ...
                            /** update activity to 0 */
                            $gUser -> ChgField('active', 0, $id, 1);
        ...
        case 'bprofile':
            ...
            // **********************************
        case 'page':
            $pa = array('about', 'advert', 'promo', 'contactus', 'privacy', 'terms', 'help', 'userdel', 'jobs', 'jobs2');
            ...
            require CLASS_PATH . 'Model/Main/Spages.php';
        ...
        default:
            ...
            /** new members */
            /*
            $lu_array = $gUser -> GetUserList(1, -1, 'u.dt DESC', 0, 4, '', '', '', '', '', 2);
            //echo $lu_array;
            
            foreach ($lu_array as $lu_key => $lu_value)
            {
              echo "$lu_array['image'] => <br>";
              foreach ($lu_value as $lu_key_2 == 'image' => $lu_value_2)
                  {
                      echo "$lu_key_2 => $lu_value_2<br>";
                  }
            
            }
            
            $gSmarty -> assign_by_ref('lu', $lu_array);
            */
            $gSmarty -> assign_by_ref('lu', $gUser -> GetUserList(1, -1, 'u.dt DESC', 0, 4, '', '', '', '', '', '', ' ' ) );
            
            /** new jobs */
	    //foreach ($gJobs -> GetList(0,0,0,3, ''. '') as $val)
		//    foreach ($val as $key => $value) echo $value;

            //$gSmarty -> assign_by_ref('jl', $gJobs -> GetList(0, 0, 0, 3, '', ''));
	    $userip = urlencode($_SERVER['REMOTE_ADDR']);
	    $useragent = urlencode($_SERVER['HTTP_USER_AGENT']);	
	    $xml=simplexml_load_file("http://api.indeed.com/ads/apisearch?publisher=8253132128796554&q=TV&l=Los+Angeles+CA&sort=date&start=0&limit=5&userip=$userip&useragent=$useragent&v=2");
	    $jl = array();
	    foreach ($xml->results->result as $thisResult)
	    $jl[] = array('jobtitle' => $thisResult->jobtitle,
		      'jobkey' =>  $thisResult->jobkey,
		      'company' => $thisResult->company,
	    		'onclk' =>	$thisResult->onmousedown,
		      'date' => $thisResult->date);
	    $gSmarty->assign_by_ref('jl', $jl);
