...
class Users
{
    ...
    public function Add($ar = array())
    {
        ...
        $sql = 'INSERT INTO '.$this -> mTbUsers.' (pass, name, email, status'.$stq.', dt)
               VALUES(?, ?, ?, ?'.$stv.', '.mktime().')';
        //echo $sql;die;
    ...
    public function Change($uid, &$ar)
    {
        ...
        $da  = array('address', 'suite', 'city', 'state', 'zip', 'dob', 'gender', 'company', 'phone', 'fax', 'lname', 'cperson_title');
        $stq = '';
        $stv = '';
        for ($i = 0; $i < count($da); $i++)
        {
    ...
    public function SaveProfile($uid, $ar = array(), $image = '')
    {
        ...
        $da  = array('title', 'interests', 'about_me', 'prof1', 'prof2', 'prof3', 'person_title', 'public_mail');
        $stq = '';
        $stv = '';
        for ($i = 0; $i < count($da); $i++)
        {
     ...
     */
    public function &GetUserList($active = 1, $status = -1, $sort = '', $first = 0, $cnt = 0, $with_admin = 0, $state = '', $city = '', $prof = 0, $letter = '', $featured = 0)
    {
    ...
    {
        $sql = 'SELECT uid, name, lname, email, dob, image, subdir, status, company, person_title FROM '.$this -> mTbUsers.' WHERE uid = uid';
        ...
        if ($sstr)
        {
            ...
            for ($i = 0; $i < count($sa); $i++)
            {
                ...
                $sq .= ($sq ? ' OR ' : '') . 'LOWER(name) LIKE "%'.$sa[$i].'%" OR LOWER(lname) LIKE "%'.$sa[$i].'%" OR LOWER(person_title) LIKE "%'.$sa[$i].'%" OR LOWER(company) LIKE "%'.$sa[$i].'%"';  
            }
            $sql .= $sq ? ' AND ('.$sq.')' : '';
        }
    ...
    public function UpdBlockUser( $uid, $uid_bl )
    {
        ...
        if ($id)
        {
            $sql = 'DELETE FROM '.$this -> mTbUsersBlock.' WHERE uid = ? AND uid_bl = ?';
            $db  = $this -> mDbPtr -> query( $sql, array($uid, $uid_bl) );
        }
        else
        {
            $sql = 'INSERT INTO '.$this -> mTbUsersBlock.' (uid, uid_bl) VALUES(?, ?)';
            $this -> mDbPtr -> query( $sql, array($uid, $uid_bl) );
        }
        return true;
    }/** UpdBlockUser */
